...
def genNetwork(input_tensor, labels_tensor, sizes):
    ...
    for i in range(1, num_layers-1):
        ...
        W = Tensor(0.01*np.random.rand(sizes[i], sizes[i-1]+1), update_rule = lambda x, y: x-learning_rate_factor*y)
        params_list.append(W)
        network = LeakyReLu(MatmulWBias(W,network))
    W = Tensor(0.01*np.random.rand(sizes[num_layers-1], sizes[num_layers-2]+1), update_rule = lambda x, y: x-y)
    network = MatmulWBias(W,network)
    network = ErrorWithNormalizationTerms(SoftmaxWithLogit(labels_tensor, network), 0.02)
...
def trainNetwork(network, learning_rate, num_epochs, do_tensor_update, labels_tensor, inputs_tensor):
    ...
    for j in range(num_epochs):
        for k in range(DATA_NUM_TRAIN):
            ...
            network.finalize()
            if i%2000 == 0:
                all_accuracy = allDataAccuracy(network, inputs_tensor, labels_tensor)
...
train_specs = trainNetwork(network, 0.001,
             1,
             do_tensor_update = lambda: updateTensors(L,X,genBatch(batch_size,train_data,train_labels)),
             labels_tensor = L,
             inputs_tensor = X)
